...
class Solution(object):
    def intersection(self, nums1, nums2):
        ...
        :rtype: List[int]
        """
        # Convert nums1 to a set to store unique elements
        s = set(nums1)
        
        # Create an empty list to store the result (intersection)
        ans = []
        
        # Iterate through nums2 and check if elements are in set s
        for num in nums2:
            if num in s:
                ans.append(num)  # Add to result list
                s.remove(num)    # Remove to prevent duplicates
