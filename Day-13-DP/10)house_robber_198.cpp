...
class Solution(object):
    def rob(self, nums):
        ...
        dp[1] = nums[0]  # Only the first house to rob

        # Fill the dp array
        for i in range(2, n + 1):
            val1 = nums[i - 1] + dp[i - 2]  # Rob current house
            val2 = dp[i - 1]                # Do not rob current house
            dp[i] = max(val1, val2)

        return dp[n]  # Return the maximum amount that can be robbed